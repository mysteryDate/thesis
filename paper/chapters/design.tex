...
The drag to connect gesture is common among similar interfaces (\citeNP{patchage}, \shortciteNP{integra}), along with Maxmapper itself. Though more advanced to program than the improvements listed above, it was seen as necessary to get libmapper users to switch to from Maxmapper to our GUI.
...
Users can also depress the ``shift'' key to select multiple rows simultaneously, a functionality found in other list interfaces like the Windows and Macintosh file browsers. Clicking anywhere in the container except for the tables de-selects all currently selected rows and arrows.
